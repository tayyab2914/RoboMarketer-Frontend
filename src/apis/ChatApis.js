...
export const API_CREATE_CHANNEL = async ( token, channelName ) => {
    // setShowSpinner(true);
    try {
        const response = await axios.post(`${DOMAIN_NAME}/chat/create_channel/`, 
            {name:channelName}
        , {
            headers: {
                Authorization: token,
...
console.log(id,name)
    try {
        const response = await axios.put(`${DOMAIN_NAME}/chat/update_channel/${id}/`, 
            {name:name}
        , {
            headers: {
                Authorization: token,
...
export const API_DELETE_CHANNEL = async (token, id) => {
    // setShowSpinner(true);
    try {
          const response = await axios.delete(`${DOMAIN_NAME}/chat/delete_channel/${id}/`, {
            headers: {
                Authorization: token, 
            },
          });
