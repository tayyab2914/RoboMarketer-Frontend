...
const dispatch = useDispatch()
  ...
  const fetchChannels = async () => {
    try {
      ...
      if (fetchedChannels.length > 0) {
        ...
        const existingChannel = fetchedChannels.find((ch) => ch.id === channel?.id);
        if (existingChannel) {
        ...
        <div className="channel-dropdown">
          <div className="channel-btn-wrapper">
            <div className="channel-btn">
              <Button onClick={handleNewChannel} style={{ fontWeight: 600, height: "40px", width: "100%" }}>
                + Add New Channel
              </Button>
            </div>
          </div>
  ...
  return (
    <div className="channels-popup">
      ...
      onVisibleChange={setDropdownVisible} overlay={dropdownMenu}  trigger={["click"]} placement="bottom" overlayStyle={{  maxWidth: "450px",minWidth:"450px " }}>
