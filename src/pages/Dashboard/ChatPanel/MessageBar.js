...
import { setRerenderChatPanel, setTemporaryMessage, setTemporaryResponse, setRerenderDashboard } from "../../../redux/AuthToken/Action";
import { API_GET_PROMPTS, API_GET_RESPONSE } from "../../../apis/ChatApis";
import { HIGHLIGHT_AT_TEXT, RENDER_FILE_PREVIEW, SHOW_API_NOT_SETUP_ERROR, SHOW_ERROR } from "../../../utils/Methods";
import "./styles/MessageBar.css";
import UpdateAccessComponent from "../UpdateAccessComponent";
import Banners from "./Banners";

const MessageBar = ({ selectedChannel, limitEnded_Redo, setLimitEnded_Redo, isAIResponseLoading }) => {
  ...
  useEffect(() => {
    if (selectedPromptRef.current) {
      selectedPromptRef.current.scrollIntoView({ behavior: "smooth", block: "nearest",});
    }
  ...
  const handleInputChange = (e) => {
    ...
    if (atIndex !== -1) {
      ...
      const filtered = prompts .filter( (prompt) => prompt.prompt_hashtag.toLowerCase().includes(searchStr) || prompt.prompt_name.toLowerCase().includes(searchStr) ) .slice(0, 5);
  ...
  const handleKeyDown = (e) => {
    if (showPromptSuggestions && filteredPrompts.length > 0) {
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setSelectedSuggestionIndex( (prev) => (prev + 1) % filteredPrompts.length );
      } else if (e.key === "ArrowUp") {
        e.preventDefault();
        setSelectedSuggestionIndex((prev) => prev === 0 ? filteredPrompts.length - 1 : prev - 1 );
      } else if (e.key === "Enter") {
        e.preventDefault();
  ...
  const selectPrompt = (prompt) => {
    if (mentionStart !== -1) {
      ...
      const newMsg = message.slice(0, mentionStart) + "@" + prompt.prompt_hashtag + " " + message.slice(mentionEnd);
  ...
  const handleSendMessage = async () => {
    ...
    if (message.trim() || file) {
      ...
      while ((match = mentionRegex.exec(message)) !== null) {
        ...
        const foundPrompt = prompts.find( (p) => p.prompt_hashtag.toLowerCase() === mentionHashtag );
      ...
      try {
        const response = await API_GET_RESPONSE( token, message, formData, setShowSpinner );
      ...
      <Row align="middle" className="message-bar">
        ...
        <Col>
          <label htmlFor="file-upload">
            <div className={`message-bar-plus ${ isFileUploading ? "disabled" : "" }`} style={{ cursor: isFileUploading ? "not-allowed" : "pointer" }} > + </div>
          </label>
        ...
        {showLimitEnded && (  <UpdateAccessComponent visible={showLimitEnded} onClose={() => { setLimitEnded(false); if (typeof setLimitEnded_Redo === "function") { setLimitEnded_Redo(false); } }}  modal={true} />  )}
        <Col>
          {isShowSpinner ? (
            <div className="message-arrow-up"> <Spin size="small" style={{ color: "#fff" }} /> </div>
          ) : (
            <div className={`message-arrow-up ${ isFileUploading ? "disabled-icon" : "" }`} onClick={!isFileUploading ? handleSendMessage : undefined} > ↑ </div>
          )}
        </Col>
      </Row>
    </div>
