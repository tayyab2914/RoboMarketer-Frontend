...
const AddPromptBtn = () => {
  ...
  const { isLoggedIn, token, current_account, rerender_dashboard } =
    useSelector((state) => state.authToken);
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [category, setCategory] = useState(null);
  const [prompt_name, setPromptName] = useState("");
  const [prompt, setPrompt] = useState("");
  const [prompt_hashtag, setPromptHashtag] = useState("");
      ...
      >
        <div className="custom-modal-header">
          <span className="modal-header">
            {" "}
            <MyIcon type="note" style={{ marginRight: "5px" }} /> Create Prompt
          </span>
          <span>
            <MyIcon
              type={"close_icon"}
