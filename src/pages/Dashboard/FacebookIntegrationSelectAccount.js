...
const FacebookIntegrationSelectAccount = ({ isInIntegrationComponent, onClose }) => {
    ...
    const [showSetAccountModal, setshowSetAccountModal] = useState(!isInIntegrationComponent);
    const { isLoggedIn, token, current_account,rerender_right_panel, facebook_state, rerender_dashboard } = useSelector( (state) => state.authToken );
    const columns = [ { title: "Account Name", dataIndex: "name", key: "name", }, { title: "Account ID", dataIndex: "account_id", key: "account_id", }, ];
    const rowSelection = { type: "radio",  selectedRowKeys, onChange: (selectedKeys, selectedRows) => { setSelectedRowKeys(selectedKeys);  setSelectedAccount(selectedRows[0]);  }, };
...
const INNER_CONTENT = ()=><>  {isInIntegrationComponent && (
        ...
        {AccountList? (
          <>
            ...
            <span className="account-selection-submit-btn-wrapper">
              <MyButton type="filled" className={"account-selection-submit-btn"} onClick={handleAccountClick} text={"Connect"} />
