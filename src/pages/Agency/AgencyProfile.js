...
const AgencyProfile = ({ isVisible, onClose }) => {
 ...
 <>
        <Row>
            <Col xs={24}>
                ...
                <Col xs={24} md={5}> <span className="awa-heading"> <MyIcon type={"profile"} /> Profile </span> </Col>
          ...
          <Form.Item label="Password" name="password" rules={PASSWORD_RULES_REQUIRED} required={false} className="form-item" style={{margin:"20px 0px"}} >
            <div className="input-group">
              <span className="input-group-input">
                    <Input.Password className="inline-input-agency" placeholder="Password" value={password} height={60} onChange={(e) => setPassword(e.target.value)} />
              </span>
              <span className="input-group-btn">
                <Button type="primary" className="inline-input-agency-btn" onClick={handlePasswordUpdate} > {" "} Update{" "} </Button>
              </span>
            </div>
          </Form.Item>
        </Form>
