...
import { Modal, Input, Button,  Upload, Form, Row, Col, Popconfirm,  } from "antd";
...
const InviteTeamMembersModal = ({ isVisible, onClose }) => {
  ...
  useEffect(()=>{
    setInvitedMembers(dummyData)
    console.log(InvitedMembers)
  },[])
  
  const handleSubmit = () => {
    const newMember = { id: InvitedMembers.length + 1, name: "New Member", email: Email, image: "https://via.placeholder.com/150" };
      ...
      <div className="custom-modal-header" style={{ display: "flex" }}>
        ...
        {InvitedMembers.map((item) => (
            ...
            <span className="invited-member-row-inner">
                ...
                <span >
                    <div style={{ marginLeft: 10 }} className="invited-member-name">{item.name}</div>
                    <div style={{ marginLeft: 10 }} className="invited-member-email">{item.email}</div>
                </span>
            </span>
            <Popconfirm title="Are you sure you want to remove this request?" onConfirm={handleDelete} okText="Yes" cancelText="No" >
